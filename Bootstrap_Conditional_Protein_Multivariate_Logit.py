...
##### PGD ~ protein + cohort + set
print('##### PGD ~ proteins')

cohort="integrated"
X = pd.read_csv('../../data/integrated_X_all_but_immunoglobulin_proteins.csv',index_col=0).apply(lambda x : (x - min(x))/(max(x) - min(x)),axis=0)
...
output.to_csv(data_dir+cohort+'/logit_bootstrap_pgd_~_all_proteins.csv')
err_df.to_csv(data_dir+cohort+'/logit_bootstrap_pgd_~_all_proteins_lwr_mean_median_upr.csv')
